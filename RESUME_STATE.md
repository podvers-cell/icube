...
Ten commits sit on local `main`, **ahead of origin and not yet pushed**. Nothing in this batch is
live. Vercel deploys on push, so pushing is the production deploy — it needs the owner's approval.
...
- TypeScript clean; 51 tests across 10 files pass (was 28 across 7).
...
1. **Push to origin.** Ten commits are local. Pushing triggers the Vercel production deploy.
2. **Delete `UPLOAD_API_KEY` and `NEXT_PUBLIC_UPLOAD_API_KEY` from the Vercel environment.** Treat
   the value as compromised. Nothing in the code reads it.
3. **Check Firebase Auth for `admin@icube.ae`.** If it exists and belongs to nobody, delete it; if
...
4. **Deploy the Firestore rules.** Still blocked: the local service account lacks
...
5. **Firestore TTL policy on `webhook_events.received_at`** (30 days suggested), or that collection
   grows without bound. Needs console access.
6. **Turnstile keys** for the contact-form abuse control (plan item 13).
7. **Verify video upload against production.** Vercel caps a function's request body at 4.5 MB by
...
1. Push and verify live, once approved.
2. Watch the CSP violation reports, then promote `Content-Security-Policy-Report-Only` to
   `Content-Security-Policy`.
3. Public rental catalogue page and the Rent Equipment link — the outstanding feature. Use
   `next/image` rather than `<img>` (plan item 20); the dashboard's plain `<img>` is fine for an
   admin table but not for a public gallery.
4. Contact-form abuse control once Turnstile keys exist (plan item 13).
5. Shared-store rate limiting or a Cloudflare WAF (plan item 15). The in-process limiter is
