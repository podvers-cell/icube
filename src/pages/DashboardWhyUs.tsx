...
export default function DashboardWhyUs() {
  ...
  return (
    <div>
      <h1 className="text-3xl font-display font-bold text-white mb-8">Why Us</h1>
      <div className="space-y-4">
        {list.map((w) => (
          <div key={w.id} className="bg-icube-gray border border-white/10 rounded-sm p-4 flex justify-between items-start">
            ...
            <button onClick={() => setEditing({ ...w })} className="px-3 py-1.5 text-sm bg-icube-gold text-icube-dark rounded-sm">Edit</button>
          </div>
        ))}
      </div>
      {editing && (
        <form onSubmit={save} className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-icube-gray border border-white/10 rounded-sm p-6 max-w-lg w-full space-y-4">
            <h2 className="text-xl font-display font-bold text-white">Edit</h2>
